...
class TeleportationRouter(BasicRouter):
    ...
    def perturbation(self, teleport_dct: dict, restrict_dct: dict, radius: int, vdp_dict: dict):
        """
        ...
        for key_tree, (path1, path2) in teleport_dct.items():
            ...
            if restrict_dct:
                res_nodes = []
                for key_st, (p1, p2) in restrict_dct.items():
                    res_nodes += p1
                    res_nodes += p2[1:]
                g_temp.remove_nodes_from(res_nodes)
            g_temp.remove_nodes_from(self.factory_pos)
    ...
    #    include_idle_teleport: bool = False, 
      # , danger_qubits: dict, available_gaps: list):
        ...
        for step in range(max_iters):
            candidate, g_temp_temp = self.perturbation(teleport_dct, restrict_dct, radius, vdp_dict)
        ...
        while len(layers) != 0:
            ...
            if include_steiner_teleport:
                ...
                if len(steiner_dct) == 0:
                    logger.info("No Steiner initialized")
                    best_steiner_init = {}
                else:
                    (
                        best_steiner_init,
            ...
            if include_idle_teleport:
                idle_move_dct = self.initialize_idle_moves(
                    vdp_dict, best_steiner_init, max_idle_teleport, layers = layers_idle, k_lookahead = k_idle
                )
                if len(idle_move_dct) == 0:
                    logger.info("No idle teleportation initialized")
                    best_idle_init = {}
                else:
                    (
                        best_idle_init,
